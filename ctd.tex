...
In Magnetic Resonance Imaging (MRI), the MRI device performs a physical two-dimensional Fourier transform of slices of a patient's body. That is, if $\rho(x)$ is the density distribution of a certain slice of a patient, then the MRI returns the data $\hat{\rho}(k)$ at various points $k$. However, the number of sample points available is limited, so it is desirable to make use of the limited information as best as possible.

A method we are currently developing for this problem involves making the assumption that $\rho(x)$ is piecewise continuous. The problem of reconstruction can then be decomposed into finding and reconstructing the edges (discontinuities), and reconstructing the smooth remainder. The filtered DFT is sufficient to reconstruct the smooth remainder, so the only problem is reconstructing the edges.

Using directional edge detectors on the Fourier-domain data, we can extract a set of points on the edges together with unoriented normals.  By means of Algorithm \ref{algo:polygonalizationNoisy}, we can reconstruct the topology of the edge set. Given this data, we can reconstruct the edges and henceforth the discontinuous part of the image. The details of this algorithm are beyond the scope of this article, but Figure \ref{fig:mriExample} illustrates the idea behind the method. Our work on curve reconstruction was motivated by this application.
...
\vspace{.1in}
  ...
  \item For each point $\vp_{i}$, iterate over the points $\vp_{j}$ contained in the node containing $\vp_{i}$ and all of it's nearest neighbors. If
    \begin{equation*}
...
\newblock In {\em Acta Informatica}, 4 (1): 1–9, 1974 doi:10.1007/BF00288933.
