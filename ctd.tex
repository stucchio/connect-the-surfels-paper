...
\newcommand{\ImgTex}{{\Img_{\textrm{tex}}}}
\newcommand{\Imgk}{\widehat{\Img}}
\newcommand{\ImgCoeff}[1]{{\Img_{#1}}}
\newcommand{\step}[1]{{1_{\gamma_{#1}}}}
...
work that considers how much better the reconstruction can be if
...
an unorganized set of points $\pointData$, as well as \emph{unit} tangents to the points $\tanData$, and an upper bound on the curvature of each curve in $\curveSet$. Note that the tangents have no particular orientation; making the change $\vm_{i} \rightarrow -\vm_{i}$ destroys no information. The algorithm is not given the number of curves, nor does it know which point lies on which curve.
...
Our goal here is to describe an algorithm which constructs the
polygonalization of a figure from the data described above.
...
is the set of points at distance $\pi \curvemaxi/2$ away from $p_{i}$, as used in the proof of Lemma \ref{lem:forbiddenZone}.}
...
between different curves (though $\beta$-crust algorithms will not have crossing edges). Knowledge of the forbidden zone allows us to
remove such edges.}
...
\vspace{.1in}
  ...
  \item Remove duplicates, since each edge is output twice at the end of step 3.
  \end{enumerate}
...
\begin{lemma}
  ...
  A curve of bounded curvature can be locally parameterized, specifically $\gamma_{i}(t)$ satisfying \eqref{eq:curvatureAssumption} admits the local parameterization
  \begin{equation}
    \label{eq:localGraphParameterization}
    \gamma_{i}(t) = \gamma(t_{0}) + (t-t_{0})\gamma'(t_{0}) + w(t) \gamma'^{\perp}(t_{0})
  \end{equation}
  where $w'(t_{0})=0$. The parameterization is valid for $\abs{t-t_{0}} < \curvemaxi$. In particular, $w'(t) < f^{-1}(\curvemax t) $ where $f(z)=z / \sqrt{1+z^{2}}$ and where $f^{-1}(z)$ denotes the functional inverse of $f(z)$. The parameterization described here is not an arc length parameterization.
\end{lemma}
...
\ifx\plotpoint\undefined\newsavebox{\plotpoint}\fi
\sbox{\plotpoint}{\rule[-0.200pt]{0.400pt}{0.400pt}}%
\includegraphics[scale=0.5]{forbidden_zone_noisy.eps}
\caption{The noisy forbidden zones allow curves to pass through at multiple angles. The light blue region represents the noisy allowed region described in definition \ref{def:AllowedRegionNoisy}.}
\label{fig:forbiddenZone}
\end{figure}
...
wavefronts.
\newblock {\em Submitted}, arxiv.org/abs/0909.5453
